...
import (
	...
	"news-management-service/internal/handlers"
...
func main() {
	...
	logger.Info("Starting News Management Service")
	...
	sourceRepo := repository.NewSourceRepository(db, logger)
	countryRepo := repository.NewCountryRepository(db, logger)
	...
	handler := handlers.NewHandler(newsService, logger)
	...
	if cfg.Metrics.Enabled {
		...
		metricsMux.HandleFunc(cfg.Metrics.Path, func(w http.ResponseWriter, r *http.Request) {
			...
			fmt.Fprintf(w, "# News Management Service Metrics\n")
			fmt.Fprintf(w, "# TYPE news_management_service_info gauge\n")
			fmt.Fprintf(w, "news_management_service_info{version=\"1.0.0\"} 1\n")
			...
			if enabled, ok := cacheStats["enabled"].(bool); ok && enabled {
				if totalItems, ok := cacheStats["total_items"].(int); ok {
					fmt.Fprintf(w, "# TYPE news_cache_items gauge\n")
					fmt.Fprintf(w, "news_cache_items %d\n", totalItems)
				}
			}
		})
	...
	logger.Info("News Management Service started successfully")
	...
	logger.Info("Shutting down News Management Service...")
	...
	logger.Info("News Management Service stopped")
}
