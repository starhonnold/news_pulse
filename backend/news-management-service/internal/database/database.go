...
func (db *DB) HealthCheck() map[string]interface{} {
	...
	var count int
	if err := db.QueryRowContext(ctx, "SELECT COUNT(*) FROM news WHERE is_active = true").Scan(&count); err != nil {
		result["query_error"] = err.Error()
	} else {
		result["active_news_count"] = count
	}
...
// Migrate проверяет наличие необходимых таблиц
func (db *DB) Migrate() error {
	db.logger.Info("Checking database schema")
	...
	db.logger.Info("Database schema check completed")
	return nil
}
