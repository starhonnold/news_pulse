...
func (r *FeedRepository) buildFeedQuery(pulse *models.UserPulse, req models.FeedRequest) (string, []interface{}) {
	...
	// Фильтр по ключевым словам пульса (логика ИЛИ)
	if pulse.Keywords != "" {
		...
		for _, keyword := range keywords {
			...
			if keyword != "" {
				// Поиск в названии, описании и контенте
				keywordConditions = append(keywordConditions, fmt.Sprintf("(n.title ILIKE $%d OR n.description ILIKE $%d OR n.content ILIKE $%d)", argIndex, argIndex+1, argIndex+2))
		...
		if len(keywordConditions) > 0 {
			conditions = append(conditions, fmt.Sprintf("(%s)", strings.Join(keywordConditions, " OR ")))
		}
	}
...
func (r *FeedRepository) determineMatchReason(pulse *models.UserPulse, news models.PersonalizedNews) string {
	...
	if pulse.Keywords != "" {
		...
		for _, keyword := range keywords {
			keyword = strings.TrimSpace(keyword)
			if keyword != "" {
				// Проверяем наличие ключевого слова в названии, описании или контенте
				if strings.Contains(strings.ToLower(news.Title), strings.ToLower(keyword)) ||
					strings.Contains(strings.ToLower(news.Description), strings.ToLower(keyword)) {
		...
		if len(matchedKeywords) > 0 {
			reasons = append(reasons, fmt.Sprintf("ключевые слова: %s", strings.Join(matchedKeywords, ", ")))
		}
	}
